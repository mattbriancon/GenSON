...
def run(args: argparse.Namespace):
    ...
    iterables = []
    if args.object:
        iterables.append(args.object)
    if args.glob:
        iterables.append(args.glob)

    for path in itertools.chain(*iterables):
        with open(path, "r") as fp:
...
def main():
    ...
    parser.add_argument(
        ...
        metavar="PATTERN",
        type=glob.iglob,
        help="glob pattern use to match files containing JSON objects",
    )
    parser.add_argument(
        "object",
